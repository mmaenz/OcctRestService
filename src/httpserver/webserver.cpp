...
webserver::webserver(const create_webserver& params) :
		port(params._port), start_method(params._start_method), max_threads(
				params._max_threads), max_connections(params._max_connections), memory_limit(
				params._memory_limit), content_size_limit(
				params._content_size_limit), connection_timeout(
				params._connection_timeout), per_IP_connection_limit(
				params._per_IP_connection_limit), log_access(
				params._log_access), log_error(params._log_error), validator(
				params._validator), unescaper(params._unescaper), bind_address(
				params._bind_address), bind_socket(params._bind_socket), max_thread_stack_size(
				params._max_thread_stack_size), use_ssl(params._use_ssl), use_ipv6(
				params._use_ipv6), debug(params._debug), pedantic(
				params._pedantic), https_mem_key(params._https_mem_key), https_mem_cert(
				params._https_mem_cert), https_mem_trust(
				params._https_mem_trust), https_priorities(
				params._https_priorities), cred_type(params._cred_type), digest_auth_random(
				params._digest_auth_random), nonce_nc_size(
				params._nonce_nc_size), running(false), default_policy(
				params._default_policy), basic_auth_enabled(
				params._basic_auth_enabled), digest_auth_enabled(
				params._digest_auth_enabled), regex_checking(
				params._regex_checking), ban_system_enabled(
				params._ban_system_enabled), post_process_enabled(
				params._post_process_enabled), deferred_enabled(
				params._deferred_enabled), single_resource(
				params._single_resource), tcp_nodelay(params._tcp_nodelay), not_found_resource(
				params._not_found_resource), method_not_allowed_resource(
				params._method_not_allowed_resource), internal_error_resource(
...
void webserver::request_completed(void *cls, struct MHD_Connection *connection,
		void **con_cls, enum MHD_RequestTerminationCode toe) {
	details::modded_request* mr =
			static_cast<details::modded_request*>(*con_cls);
	if (mr == 0x0)
		return;
...
bool webserver::register_resource(const std::string& resource,
		http_resource* hrm, bool family) {
	if (single_resource && ((resource != "" && resource != "/") || !family)) {
		throw std::invalid_argument(
				"The resource should be '' or '/' and be marked as family when using a single_resource server");
	}

	details::http_endpoint idx(resource, family, true, regex_checking);

	pair<map<details::http_endpoint, http_resource*>::iterator, bool> result =
			registered_resources.insert(
					map<details::http_endpoint, http_resource*>::value_type(idx,
							hrm));

	if (result.second) {
		registered_resources_str.insert(
				pair<string, http_resource*>(idx.get_url_complete(),
						result.first->second));
	}
...
bool webserver::start(bool blocking) {
	...
	struct {
		MHD_OptionItem operator ()(enum MHD_OPTION opt, intptr_t val,
				void *ptr = 0) {
	...
	iov.push_back(
			gen(MHD_OPTION_NOTIFY_COMPLETED, (intptr_t) &request_completed,
			NULL));
	...
	iov.push_back(
			gen(MHD_OPTION_UNESCAPE_CALLBACK, (intptr_t) &unescaper_func,
					this));
	...
	if (per_IP_connection_limit != 0)
		iov.push_back(
				gen(MHD_OPTION_PER_IP_CONNECTION_LIMIT,
	...
	if (use_ssl)
		iov.push_back(
				gen(MHD_OPTION_HTTPS_MEM_KEY, 0,
						(void*) https_mem_key.c_str()));
	if (use_ssl)
		iov.push_back(
				gen(MHD_OPTION_HTTPS_MEM_CERT, 0,
						(void*) https_mem_cert.c_str()));
	if (https_mem_trust != "" && use_ssl)
		iov.push_back(
				gen(MHD_OPTION_HTTPS_MEM_TRUST, 0,
						(void*) https_mem_trust.c_str()));
	if (https_priorities != "" && use_ssl)
		iov.push_back(
				gen(MHD_OPTION_HTTPS_PRIORITIES, 0,
						(void*) https_priorities.c_str()));
	if (digest_auth_random != "")
		iov.push_back(
				gen(MHD_OPTION_DIGEST_AUTH_RANDOM, digest_auth_random.size(),
	...
	if (this->daemon == NULL) {
		if (bind_address == 0x0) {
			this->daemon = MHD_start_daemon(start_conf, this->port,
					&policy_callback, this, &answer_to_connection, this,
					MHD_OPTION_ARRAY, &iov[0], MHD_USE_DEBUG, MHD_OPTION_END);
		} else {
			this->daemon = MHD_start_daemon(start_conf, 1, &policy_callback,
					this, &answer_to_connection, this, MHD_OPTION_ARRAY,
					&iov[0], MHD_OPTION_SOCK_ADDR, bind_address,
					MHD_OPTION_END);
		}
	}
	if (this->daemon == NULL) {
		...
		throw std::invalid_argument(
				"Unable to connect daemon to port: "
...
int policy_callback(void *cls, const struct sockaddr* addr, socklen_t addrlen) {
	...
	if ((((static_cast<webserver*>(cls))->default_policy == http_utils::ACCEPT)
			...
			|| (((static_cast<webserver*>(cls))->default_policy
					== http_utils::REJECT)
					&& ((!(static_cast<webserver*>(cls))->allowances.count(addr))
							|| ((static_cast<webserver*>(cls))->bans.count(addr))))) {
		return MHD_NO;
...
const std::shared_ptr<http_response> webserver::not_found_page(
		details::modded_request* mr) const {
	if (not_found_resource != 0x0) {
		return not_found_resource(*mr->dhr);
	} else {
		return std::shared_ptr<http_response>(
				new string_response(NOT_FOUND_ERROR,
						http_utils::http_not_found));
	}
}

const std::shared_ptr<http_response> webserver::method_not_allowed_page(
		details::modded_request* mr) const {
	if (method_not_allowed_resource != 0x0) {
		return method_not_allowed_resource(*mr->dhr);
	} else {
		return std::shared_ptr<http_response>(
				new string_response(METHOD_ERROR,
						http_utils::http_method_not_allowed));
	}
}

const std::shared_ptr<http_response> webserver::internal_error_page(
		details::modded_request* mr, bool force_our) const {
	if (internal_error_resource != 0x0 && !force_our) {
		return internal_error_resource(*mr->dhr);
	} else {
		return std::shared_ptr<http_response>(
				new string_response(GENERIC_ERROR,
						http_utils::http_internal_server_error, "text/plain"));
	}
}
...
int webserver::bodyfull_requests_answer_first_step(MHD_Connection* connection,
		struct details::modded_request* mr) {
	mr->second = true;
	...
	const char *encoding = MHD_lookup_connection_value(connection,
			MHD_HEADER_KIND, http_utils::http_header_content_type.c_str());
	...
	if (post_process_enabled
		...
		mr->pp = MHD_create_post_processor(connection, post_memory_limit,
				&post_iterator, mr);
	} else {
		mr->pp = NULL;
	}
...
int webserver::finalize_answer(MHD_Connection* connection,
	...
	if (!single_resource) {
		...
		if (fe == registered_resources_str.end()) {
			if (regex_checking) {
				...
				details::http_endpoint endpoint(st_url, false, false,
						regex_checking);
				...
				for (it = registered_resources.begin();
						it != registered_resources.end(); ++it) {
					size_t endpoint_pieces_len =
							(*it).first.get_url_pieces().size();
					size_t endpoint_tot_len =
							(*it).first.get_url_complete().size();
				...
				if (found) {
					vector<string> url_pars =
							found_endpoint->first.get_url_pars();
...
int webserver::complete_request(MHD_Connection* connection,
...
int webserver::answer_to_connection(void* cls, MHD_Connection* connection,
	...
	if (mr->second != false) {
		return static_cast<webserver*>(cls)->bodyfull_requests_answer_second_step(
				connection, method, version, upload_data, upload_data_size, mr);
	}
	...
	access_log(static_cast<webserver*>(cls),
	...
	if (0 == strcasecmp(method, http_utils::http_method_get.c_str())) {
		...
		body = true;
			...
			== strcasecmp(method, http_utils::http_method_delete.c_str())) {
		...
		mr->callback = &http_resource::render_HEAD;
			...
			== strcasecmp(method, http_utils::http_method_connect.c_str())) {
		...
		mr->callback = &http_resource::render_TRACE;
			...
			== strcasecmp(method, http_utils::http_method_options.c_str())) {
		mr->callback = &http_resource::render_OPTIONS;
	}

	return body ?
			static_cast<webserver*>(cls)->bodyfull_requests_answer_first_step(
					connection, mr) :
			static_cast<webserver*>(cls)->bodyless_requests_answer(connection,
					method, version, mr);
}
